...
this to O(sqrt(t log t)). The problem: improve the simulation. Two concrete
...
problem touch a lot of edges. It is also the showcase for catalytic/clean-tape
techniques crossing into classical structural complexity.

## Attack surface

Route 1: improve Tree Evaluation to O(log n) space (see the companion problem
...
error in its height-compression framework, so the problem's gap is untouched.
