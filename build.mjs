...
for (const g of order) {
  ...
  for (const c of group) {
    idxHtml += `<a class="card-link" href="problems/${c.id}.html"><span class="t">${esc(c.title)}</span>${chips(c)}</a>`;
    idxMd += `| [${c.id}](problems/${c.id}.md) | ${c.status} | ${c.confidence} | ${c.title.replace(/\|/g, '\\|')} |\n`;
  }
}
