// Build the static site (docs/) and INDEX.md from problems/*.md frontmatter.
// Usage: node build.mjs
import { marked } from 'marked';
import { readFileSync, writeFileSync, readdirSync, mkdirSync, rmSync } from 'fs';
import { join, basename } from 'path';

const CARDS_DIR = 'problems';
const OUT_DIR = 'docs';
...
const cards = readdirSync(CARDS_DIR).filter(f => f.endsWith('.md')).map(f => {
  ...
  if (!meta.endgame) throw new Error(`missing endgame in ${f}`);
  const spine = !/^none known/i.test(meta.endgame);
  return { ...meta, body, file: f, spine };
});
...
.chip.open { color: var(--open); } .chip.verified { color: var(--accent); }
.chip.spine { color: var(--accent); font-weight: 600; }
.endgame-line { font-size: .82rem; color: var(--muted); margin-top: .2rem; }
...
function chips(c) {
  const out = [`<span class="chip ${c.status}">${esc(c.status)}</span>`];
  if (c.spine) out.push(`<span class="chip spine">&#10227; loop-capable</span>`);
  out.push(`<span class="chip">${esc(GENRES[c.genre])}</span>`, `<span class="chip">confidence: ${esc(c.confidence)}</span>`);
...
mkdirSync(join(OUT_DIR, 'problems'), { recursive: true });
...
for (const c of cards) {
  ...
  writeFileSync(join(OUT_DIR, 'problems', `${c.id}.html`), page(c.title, body));
}

// Index page + INDEX.md
const order = Object.keys(GENRES);
cards.sort((a, b) => order.indexOf(a.genre) - order.indexOf(b.genre) || a.id.localeCompare(b.id));
const spineCards = cards.filter(c => c.spine);
const webCards = cards.filter(c => !c.spine);
const tally = `${cards.length} open problems`;
let idxHtml = `<h1>Open problems in complexity theory</h1>
<p class="tagline">${tally}, reduction-shaped and each anchored to a published record.
Beat the number on any of them and you have a publishable result. Records are community-maintained; corrections are contributions.</p>`;
let idxMd = `# Problem Index\n\n${tally}, each anchored to a published record. Generated by \`node build.mjs\` — do not edit by hand.\n`;

const entryHtml = c => `<a class="card-link" href="problems/${c.id}.html"><span class="t">${esc(c.title)}</span>${chips(c)}${c.spine ? `<div class="endgame-line">&#10227; ${esc(c.endgame)}</div>` : ''}</a>`;
const entryMd = c => `| [${c.id}](problems/${c.id}.md) | ${c.status} | ${c.confidence} | ${c.title.replace(/\|/g, '\\|')} |\n`;

idxHtml += `<h2>The loop-capable spine (${spineCards.length})</h2>
<p class="tagline">Problems with a <em>published</em> path from their record to an unconditional result —
a collision with a hierarchy theorem, the algorithmic method, or hardness magnification.
Progress here provably accumulates toward separations.</p>`;
idxMd += `\n## The loop-capable spine (${spineCards.length})\n\nProblems with a published path from their record to an unconditional result.\n\n| Problem | Status | Confidence | Challenge |\n|---|---|---|---|\n`;
for (const c of spineCards) { idxHtml += entryHtml(c); idxMd += entryMd(c); }

idxHtml += `<h2>The wider web (${webCards.length})</h2>
<p class="tagline">No published path to an unconditional result — yet. These tighten the web of reductions,
and finding an endgame for one of them is itself a contribution.</p>`;
idxMd += `\n## The wider web (${webCards.length})\n\nNo published endgame path known — finding one is itself a contribution.\n`;
for (const g of order) {
  const group = webCards.filter(c => c.genre === g);
  if (!group.length) continue;
  idxHtml += `<h3>${esc(GENRES[g])} (${group.length})</h3>`;
  idxMd += `\n### ${GENRES[g]} (${group.length})\n\n| Problem | Status | Confidence | Challenge |\n|---|---|---|---|\n`;
  for (const c of group) { idxHtml += entryHtml(c); idxMd += entryMd(c); }
}
writeFileSync(join(OUT_DIR, 'index.html'), page('Open problems in complexity theory', idxHtml));
writeFileSync('INDEX.md', idxMd);
console.log(`built ${cards.length} problems -> ${OUT_DIR}/, INDEX.md regenerated`);
